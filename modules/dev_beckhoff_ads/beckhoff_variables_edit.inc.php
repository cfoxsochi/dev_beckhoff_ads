...
*/
  ...
  if ($this->tab=='data') {
   ...
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM beckhoff_variables_data WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM beckhoff_variables_data WHERE variable_id='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('beckhoff_variables_data', $properties[$i]);
